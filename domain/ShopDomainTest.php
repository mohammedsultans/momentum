...
class CatalogTest extends PHPUnit_Framework_TestCase
{
  ...
  public function testBuyingProcess() {
	...
	//auto instatiates sales agreement in SESSION OBJECT*****

	//$saleagreement = new ConnectedAccountabilityType('SaleAgreement');
 	//$saleagreement->addConnectionRule(new PartyType('Customer'), new PartyType('Vendor'));

 	//$session = new ShoppingSession();
	
	//new ShoppingSession();
	//sale stored in static variable 
	//timestamp - when opened, when ordered, when closed, when paid, days overdue, interests accrued, post amounts?, discounting, 
	//creates empty order object and returns sale object with Order-ID
	//$sale = SaleAccountability::GetAgreement($saleid);
